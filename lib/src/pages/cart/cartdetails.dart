...
class CartDetailsWidget extends StatefulWidget {
  ...
  CartDetailsWidget({Key key, this.scaffoldKeyCart1, this.norush})
      : super(key: key);
...
class _CartDetailsWidgetState extends State<CartDetailsWidget> {
  ...
  bool checkBool = false, special = false;
    ...
    if (isLogin) {
      ...
      if (res != null) {
        ...
        if (res["status"] && (res["data"]["products"]?.isNotEmpty ?? false)) {
          print(res["data"]["totalPrice"].toString());
          // double p = res["data"]["totalPrice"];
          print("special api--");
          Map specialRes = {};
          await hp
              .post(baseUrl + specialEndPoint,
                  body: json.encode({
                    "price":
                        // res["data"]["SubTotal"]
                        res["data"]["totalPrice"] -
                            (res["data"]["totalShippingPrice"] ?? 0)
                  }),
                  headers: headers)
              .then((response) {
            // //print("after then of http" + response.toString());
            if (response != null) {
              specialRes = json.decode(response.body);
          ...
          // Map specialRes = await ApiServices.postRequest(specialEndPoint,
          //     json.encode({"price": res["data"]["totalPrice"]}));
          print("special api done--");

          if (specialRes != null) {
            setState(() {
              special = true;
              print("specialRes----------------------------------$specialRes");
              if (specialRes["data"] != null) {
                specialAmount = double.parse(
                    specialRes["data"]["cashback_amount"].toString());
              } else {
                specialAmount = 0;
              }
            });
...
//                        : Container(),
                                    ...
                                    special
                                        ? Padding(
                                            ...
                                            child: textST2(
                                                "You will be getting the approximate special event cashback  of ${Helper.getPrice(specialAmount)} form this order",
                                                // "Expected delivery on ${DateFormat.yMMMEd().format(DateTime.now().add(regularDay))}(${regularData?.deliveryHours})",
                                                textstyle: TextStyle(
                                                    color: Theme.of(context)
                                                        .primaryColor),
                                                        ...
                                                        if (stockBool ??
                                                          ...
                                                          showDialog(
                                                            ...
                                                            builder:
                                                              ...
                                                              //       child:
                                                              //           AutoSizeText(
                                                              //         "Do you want to review your cart for the price/stock changes or confirm the order with the current price(s)?",
                                                              //         maxLines:
                                                              //             3,
                                                              //         minFontSize:
                                                              //             11,
...
class CustomDialogBox extends StatefulWidget {
  List<CartVerify> cartVerifyList = [];
  int length, payment_id;
  List<dynamic> cartVerifyres = [];
  Address userObject;
  String payment_type;
  bool norush;
...
class _CustomDialogBoxState extends State<CustomDialogBox> {
  ...
  contentBox(context) {
    widget.cartVerifyList.removeWhere((v) {
      return v.statusCode == "INSTOCK";
    });
      ...
      children: <Widget>[
        Container(
          padding: EdgeInsets.only(
              left: border, top: top, right: border, bottom: border),
            ...
            children: <Widget>[
              Container(
                ...
                child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      // print("msg====");
                      // print(widget.cartVerifyList[index]);
                      if (widget.cartVerifyList[index].statusCode != "INSTOCK")
                        return SizedBox(
                          width: SizeConfig.w * 0.6,
                          child: Text(
                            widget.cartVerifyList[index].message,
                            maxLines: 3,
                            softWrap: true,
                          ),
                        );
                  ...
                  FlatButton(
                    ...
                    onPressed: () async {
                      await ApiServices.postRequestToken(
                        json.encode(widget.cartVerifyres),
                        cartConfirmEndPoint,
                      );
                  ...
                  FlatButton(
                      ...
                      onPressed: () async {
                        Map<String, dynamic> data = {
                          "billingAddress": widget.userObject.toMap(),
                          "payment_type": widget.payment_type,
                          "nextweekdelivery": widget.norush,
                        ...
                        //     "hitting api for order");
                        // Map cartRes =
                        //     await ApiServices.getRequestToken(cartEndPoint);

                        Map confirmRes = await ApiServices.postRequestToken(
                          json.encode(widget.cartVerifyres),
                          cartConfirmEndPoint,
                        );
                        print("confirm res----" + confirmRes.toString());
                        Map res = {};
                        if (confirmRes["status"])
                          res = await ApiServices.postRequestToken(
                            json.encode(data),
                            createOrderEndPoint,
                          );
                        else {
                        ...
                        if (res["status"] != null && res["status"]) {
                          ...
                          // print(" $token ");
                          try {
                            userRes = await ApiServices.getRequestToken(
                                userAddEndPoint);
                          } catch (e) {
                          ...
                          if (userRes != null) if (userRes["addresses"] !=
                                  null &&
                          ...
                          print("txnid-- Id${res["txnid"]}");

                          if (widget.payment_id == 1)

...
class _MyEarningsState extends State<MyEarnings> with TickerProviderStateMixin {
  ...
  // List<EarningData> productList;
  getEarning() async {
    showLoading();
    Map res = await ApiServices.getRequestToken(earningEndPoint + "?json=1");
    print(res);
    if (res != null) if (res["data"] == null) {
      ...
      Provider.of<MyEarningData>(context, listen: false).setData(
        earningData,
      );
      // earningData = List<EarningData>.from(
      //     res["data"]["items"]?.map((x) => EarningData.fromMap(x) ?? []));
